...
class PortScanner:
    ...
    def scan_network_range(self, network: str, ports: List[int] = None) -> dict:
        ...
        print(
            f"{Fore.BLUE}[*] Analizando {len(hosts)} posibles hosts...{Style.RESET_ALL}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
            future_to_ip = {executor.submit(
                process_host, ip): ip for ip in hosts}
            for future in concurrent.futures.as_completed(future_to_ip):
                result = future.result()
                if result:
                    ip_str, open_ports = result
                    print(
                        f"{Fore.GREEN}[+] Host activo encontrado: {ip_str} ({len(open_ports)} puertos abiertos){Style.RESET_ALL}")
...
class VulnerabilityManager:
    ...
    def close_port(port: int, pid: int = None) -> bool:
        ...
        try:
            ...
            print(
                f"{Fore.YELLOW}[!] Intentando cerrar puerto {port} (PID: {pid}, Proceso: {process_name}){Style.RESET_ALL}")

            # Solicitar confirmación
            response = input(
                f"¿Deseas terminar el proceso '{process_name}'? (s/n): ")
            if response.lower() != 's':
            ...
            print(
                f"{Fore.GREEN}[+] Puerto {port} cerrado exitosamente{Style.RESET_ALL}")
            return True
            ...
            print(
                f"{Fore.RED}[!] Acceso denegado. Ejecuta el programa como administrador{Style.RESET_ALL}")
            return False
        except Exception as e:
            print(
                f"{Fore.RED}[!] Error al cerrar puerto: {e}{Style.RESET_ALL}")
            return False
...
class MalwareScanner:
    ...
    def scan_directory(self, path: str, recursive: bool = True) -> List[dict]:
        ...
        try:
            ...
            print(
                f"{Fore.RED}[!] Permiso denegado para acceder a: {path}{Style.RESET_ALL}")
        except Exception as e:
            print(
                f"{Fore.RED}[!] Error al escanear directorio: {e}{Style.RESET_ALL}")
...
def scan_local_ports_menu():
    ...
    for port_info in open_ports:
        status = f"{Fore.RED}VULNERABLE{Style.RESET_ALL}" if port_info[
            'vulnerable'] else f"{Fore.GREEN}OK{Style.RESET_ALL}"
    ...
    for host, ports in results.items():
        ...
        for port_info in ports:
            status = f"{Fore.RED}VULNERABLE{Style.RESET_ALL}" if port_info[
                'vulnerable'] else f"{Fore.GREEN}OK{Style.RESET_ALL}"
            print(
                f"{port_info['port']:<10} {port_info['service']:<15} {status}")
...
def close_vulnerable_ports_menu():
    ...
    if choice == '1':
        try:
            ...
            port_info = next(
                (p for p in vulnerable_ports if p['port'] == port_num), None)
            if port_info:
...
def scan_malware_menu():
    ...
    for idx, file_info in enumerate(suspicious_files, 1):
        ...
        if 'error' not in file_info:
            print(
                f"  Razón: {Fore.YELLOW}{file_info.get('reason', 'Desconocida')}{Style.RESET_ALL}")
...
def main():
    ...
    if is_admin:
        print(
            f"{Fore.GREEN}[+] Ejecutando con permisos de ADMINISTRADOR. Funciones completas activas.{Style.RESET_ALL}")
    else:
        print(
            f"{Fore.YELLOW}[!] Ejecutando con permisos de usuario estándar.{Style.RESET_ALL}")
        if platform.system() == "Windows":
            print(
                f"{Fore.YELLOW}[!] Para poder cerrar puertos y procesos del sistema, ejecuta como Administrador.{Style.RESET_ALL}")
        else:
            print(
                f"{Fore.YELLOW}[!] Para poder cerrar puertos, ejecuta con sudo.{Style.RESET_ALL}")
