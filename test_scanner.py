...
def test_port_scanner():
    ...
    try:
        ...
        if open_ports:
            print(f"  Puerto de ejemplo: {open_ports[0]['port']} - {open_ports[0]['process']}")
...
def test_port_detection():
    ...
    try:
        ...
        print(f"✓ Puerto 9999 en localhost: {'Abierto' if is_open else 'Cerrado'}")
        
        # Verificar puertos vulnerables conocidos
        print(f"✓ Puertos vulnerables definidos: {len(port_scanner.VULNERABLE_PORTS)}")
